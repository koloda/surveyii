...
use app\modules\study\controllers\admin\AdminController;
...
class DefaultController extends AdminController
{
    ...
    public function actionDelete(int $id, int $eventId = null)
    {
        ...
        if (\Yii::$app->request->isAjax) {
            ...
            \Yii::$app->session->addFlash('task-success', 'Вкладка с заданиями успешно удалена');
            return $this->redirect("/tour/admin/event/update?id={$eventId}#tasks");
        }
    }
    ...
    public function actionCreate(int $eventId = null)
    {
        $survey = new Survey();
        ...
        if ($eventId) {
            $survey->event_id = $eventId;
        }

        $survey->save(false);
        \Yii::$app->session->set('surveyUploadsSubpath', $survey->survey_id);

        return $this->redirect(['update', 'id' => $survey->survey_id]);
    }
